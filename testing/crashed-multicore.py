...
if __name__ == '__main__':
    ...
    # on a q6600 doubling the pool will yield significant gains
    # this might be because of blocking from writing to the memmap file
    pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
    chunk_files = glob.glob(os.path.join('World1', '*', '*', '*.dat'))
    ...
    # intialize shared mmap array
    image_array = numpy.memmap(mmap_filen, dtype=numpy.uint8, mode='w+', shape=(map_image_size[1], map_image_size[0], 3))
    image_array[:] = (255, 255, 255)
    image_array.flush()

    end_init_array = time.time()
